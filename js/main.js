...
$(document).ready(function () {
  ...
  $("#carousel-container").attr(
    "aria-label",
    "You are navigating a sliding carousel, you are at slide " +
      counter +
      " of " +
      numslides +
      " carousel."
  ...
  $("button#go").click(function () {
    console.log(
      "Play Button pressed, turning autoplay on - AutoPlay Value is:" + autoPlay
    );
    if (autoPlay == "on") {
      ...
      //$("#go").text("Play");
      $("#go").html(
        ' <img alt="" role="presentation" src="images/16824a607db2752105ad.svg">'
      );
      ...
      if (!timer) {
        ...
        //$("#go").text("Pause");
        $("#go").html(
          ' <img alt="" role="presentation" src="images/269d61205ffd47a2ed46.svg">'
        );
      }
    }
  });
...
function shownext(numslides) {
  var current = $("#carousel-container div[aria-hidden='false']");
  var currentDot = $(
    ".app-containers-Carousel-carousel__dot--Zd7ra[aria-hidden='false']"
  );
  var slideBack = $(
    ".app-components-CrossfadeImage-crossfadeimage__fader--aXX5X[aria-hidden='false']"
  );
  ...
  if (next.length) {
    ...
    $(slideBack)
      .siblings(".app-components-CrossfadeImage-crossfadeimage__fader--aXX5X")
    ...
    $(currentDot).removeClass(
      "app-containers-Carousel-carousel__active--F2DLc"
    );
    $(currentDot).html(
      '<img alt="" role="presentation"  src="images/55ffc2fbf44c4c00c360.svg">'
    );

    $(currentDot).siblings("button").attr("aria-hidden", "true");
    $(currentDot)
      .siblings(".app-containers-Carousel-carousel__dot--Zd7ra")
      .addClass("app-containers-Carousel-carousel__active--F2DLc");
    $(currentDot)
      .siblings(".app-containers-Carousel-carousel__dot--Zd7ra")
      .html(
        '<img alt="" role="presentation"  src="images/4cdb620f13093dcffa1e.svg">'
      );
    ...
    slideBack = $(
      ".app-components-CrossfadeImage-crossfadeimage__fader--aXX5X:first"
    );
    $(slideBack).attr("aria-hidden", "false");
    $(slideBack).removeClass("hidden");
    $(slideBack)
...
$(document).ready(function () {
  ...
  selectcountryfrom1.change(function () {
    if (selectcountry.val() == selectcountryfrom1.val()) {
      alert(
        "Sorry! You can not select " + selectcountryfrom1.val() + " again."
      );
  ...
  selectcountryfrom2.change(function () {
    if (selectcountry1.val() == selectcountryfrom2.val()) {
      alert(
        "Sorry! You can not select " + selectcountryfrom2.val() + " again."
      );
  ...
  selectcountryfrom3.change(function () {
    if (selectcountry1.val() == selectcountryfrom3.val()) {
      alert(
        "Sorry! You can not select " + selectcountryfrom3.val() + " again."
      );
  ...
  $(".addMore").click(function () {
    if ($("body").find(".fieldGroup").length < maxGroup) {
      var fieldHTML =
        '<div class="fieldGroup">' + $(".fieldGroupCopy").html() + "</div>";
